...
class Salequery_handler(BaseHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        for area in all_area:
            for team in team_list:
                for group in group_list:
                    s = [area, team, group]
                    ##过滤不存在的区域、团队、组关系
                    a_count = TpyrcedSaleadd.select().where(TpyrcedSaleadd.re_area == area,TpyrcedSaleadd.re_team == team,TpyrcedSaleadd.re_group == group).count()
                    if a_count:
    		        ret.append(s)
        ...
        for area in all_area:
            ...
            ar_count = TpyrcedBidadd.select().where(TpyrcedBidadd.bid_elec == bid_date,TpyrcedBidadd.area_main == area).count()
            if ar_count > 0:
        ...
        ##过滤区域
        area_filter = request_dict['sale_area']
        select_area = []
        if area_filter[0] != "all":
            for j in area_filter:               
                for n in table_data:
        ...
        select_team = []
        if team_filter[0] != "all":
            for j in team_filter:               
                for n in table_data:
                    if (j.encode("utf8")) in n:
        ...
        select_group = []
        if group_filter[0] != "all":
            for j in group_filter:               
                for n in table_data:
                    if (j.encode("utf8")) in n:
                        select_group.append(n)
